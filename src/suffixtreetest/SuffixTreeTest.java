...
import Sufix.SuffixTree;
import java.util.Arrays;
import java.util.Collection;
import java.util.HashSet;

import static Sufix.Main.getSubstrings;
...
public class SuffixTreeTest extends TestCase {
    ...
    public void testBasicTreeGeneration() {
        ...
        for (String s : getSubstrings(word)) {
            assertTrue(in.buscar(s).contains(0));
        }
        assertEmpty(in.buscar("caco"));
        assertEmpty(in.buscar("cacaoo"));
        assertEmpty(in.buscar("ccacao"));
        ...
        assertEmpty(in.buscar("books"));
        assertEmpty(in.buscar("boke"));
        assertEmpty(in.buscar("ookepr"));
    }
    ...
    public void testDouble() {
        ...
        for (String s : getSubstrings(word)) {
            assertTrue(in.buscar(s).contains(0));
            assertTrue(in.buscar(s).contains(1));
        }
    }
    ...
    public void testBananaAddition() {
        ...
        for (int i = 0; i < words.length; ++i) {
            in.addIndex(words[i], i);

            for (String s : getSubstrings(words[i])) {
                Collection<Integer> result = in.buscar(s);
        ...
        for (int i = 0; i < words.length; ++i) {
            for (String s : getSubstrings(words[i])) {
                assertTrue(in.buscar(s).contains(i));
            }
        }

        // add again, to see if it's stable
        for (int i = 0; i < words.length; ++i) {
            in.addIndex(words[i], i + words.length);

            for (String s : getSubstrings(words[i])) {
                assertTrue(in.buscar(s).contains(i + words.length));
            }
        }
    ...
    public void testAddition() {
        ...
        in.contar();
        testResultsCount(in.getRaiz());

        assertEmpty(in.buscar("aoca"));
    }
    ...
    private void testResultsCount(Nodo n) {
        for (Link e : n.getLinks().values()) {
            assertEquals(n.getInfo().size(), n.getCount());
            testResultsCount(e.getDestino());
        }
    }
